...
// Scan `text` for the maximum {}/[] nesting depth WITHOUT invoking a
// recursive parser — a flat linear scan that respects string literals (so
// brackets inside a JSON string value are never counted) and JSON's
// backslash-escaping rules. Returns the max depth reached, or -1 if depth
// exceeds `maxDepth` (scan stops immediately at that point — cheap on an
// adversarial input, not just a valid one).
